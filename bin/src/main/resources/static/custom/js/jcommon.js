...
$(function () {
	...
	// 以下のクラス指定により、カレンダーの入力形式を「yyyyMMddHHmmss」に設定
	$('.cs-YYYYMMDDHHmmss').datetimepicker({
		locale: 'ja',
...
var richEdit = function(editButton) {
	$('div[data-name = ' + editButton.name + ']').summernote({
		focus: true,
		disableDragAndDrop: true,
		toolbar: [
		            ['misc', ['undo','redo']],
		            ['font-style', ['fontsize', 'color', 'bold', 'italic', 'underline', 'strikethrough', 'superscript', 'subscript', 'height']],
		            ['style', ['ul', 'ol', 'paragraph']],
...
jQuery(window).load(function(){
	...
	// .cs-comma-text 変更時
	$('.cs-comma-text').on('DOMSubtreeModified', function() {
		var num = $(this).text();
		var obj = $(this);
	...
	// .cs-slash-text 変更時
	$('.cs-slash-text').on('DOMSubtreeModified', function() {
		var date = $(this).text();
		var obj = $(this);
		if ($.isNumeric(date)) {
...
$('a[class = cs-carousel-link]').click(function(){
	...
	if($(this).children('input').eq(0).val() != ''){// 画像情報がない場合は何もしない.
		...
		$('#cs-carousel-mdTitle').append($(this).children('input').eq(0).val());
