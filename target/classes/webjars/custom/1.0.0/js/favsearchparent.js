...
$(function(){
	...
	$('a[class = cs-fav-search-href]').on('click', function(event){
		...
		// 入力値のクリア.
		$(this).parents('.btn-toolbar').find('button[name=clearButton]').click();
		
		var selectFavName = $(this).attr('name');
		// 選択されたお気に入り情報に紐づくお気に入り登録値情報をhidden項目から取得する.
		$(document).find('div[name=favSearchHidden]').each(function(j, favElement){
			// お気に入りの値をhidden項目の名前と一致する入力エリアに反映する.
			$(favElement).find('input').each(function(i, favInput) {
				if($(favInput).attr('class') == selectFavName){
		...
		$('input[type="hidden"][name="favSearchNo"]').val(selectFavName);
	...
	$('button[name = favSearchUpdateButton]').click(function(){
		...
		var kinouId = $('form').find('input[type=hidden][name=kinouId]').val();
		// モーダル画面呼出し.
		var url = encodeURI(favSearchPath + '/04?favSearchNo=' + favSearchNo + '&kinouId=' + kinouId);
	...
	// 【お気に入り情報】共通化する？
	// 引数：機能ID、　モーダルを閉じたあとの処理となるFUNCTIONをこのJS内で定義する感じ？
	/**
	 * お気に入りの保存ボタン押下時.
	...
	$('button[name = favSearchInsertButton]').click(function(){
        ...
        // 未入力の場合は戻る.
        if(!arrColumnNo.length){
        	alert($('input[name = favNoValueMsg]').val());
        	button.attr("disabled", false);
        	return
        }
        var form = {
        		"kinouId" : $('input[name = kinouId]').val(),// お気に入りの呼出し元画面の機能IDを設定する.
        ...
        $.ajax({
          ...
          dataType : "json", // レスポンスをJSONとしてパースする
    		...
    		var url = encodeURI(favSearchPath + "/03?kinouId=" + kinouId);
...
$(window).on('load',function(){
	...
	if(favSearchNo){
		$('a[class = cs-fav-search-href][name=' + favSearchNo + ']').click();
	}
});